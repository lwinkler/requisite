...
def generate_all_git_files_command(path: Path, extensions: List[str] ) -> None:
    """Generate a command to list all files with git+grep"""
    grep = " | grep " + ' '.join(['-e \"\\.' + extension + "$\"" for extension in extensions]) if extensions else ""
...
def list_all_git_files(path: Path, extensions: List[str] ) -> List[Path]:
    ...
    ret = subprocess.run(command, capture_output=True, encoding="utf-8", check= True, shell=True)
...
def run_on_all_git_files(command: str, path: Path, extensions: List[str] ) -> None:
    ...
    full_command = generate_all_git_files_command(path, extensions) + " | xargs " + command
